...
export const matchF = 
  ...
  (either: Either<E, T>) => 
  ("value" in either ? matchers.right(either.value) : matchers.left(either.error))

export const match = 
  <E extends Error, T, R1, R2>({ either, matchers }: { either: Either<E, T>, matchers: Matchers<E,T,R1,R2> }) =>
    ("value" in either ? matchers.right(either.value) : matchers.left(either.error))
